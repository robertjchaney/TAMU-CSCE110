...
def play_victory_sound():
    playsound("Final Project\\mp3 Files\\Happy Wheels victory green screen.mp3")
def play_dice_sound():
...
def mainMenu():
    screen.bgpic("C:\\Users\\ososb\\Downloads\\pythonProject\\Final Project\\Cash's Project\\Game Menu.gif")
...
def playMenu():
    """Pulls up the main menu for the Zanzibar Dice Game"""
    threading.Thread(target=stop_menu_music, daemon=True).start()
    bye()

def rulesMenu():
    """Pulls up the rules to the game"""
    screen = Screen()
    screen.setup(1.0, 1.0)
    screen.bgpic("C:\\Users\\ososb\\Downloads\\pythonProject\\Final Project\\Cash's Project\\Game Menu No Buttons.gif")
    ...
    def scroll_text(text_lines, start_y, step_delay=0.02, font=("Oswald", 30)):
        count = 0

        pen.clear()  # Clear any existing text
        y = start_y  # Start at the specified y-coordinate
        
        for i in range(11 * (len(text_lines) + 1)):  # Allow blank frame after last line scrolls off
            pen.clear()  # Clear the screen for smooth animation
            y += 10  # Move all text upwards
...
def creditMenu():
    """Pulls up credits and thanks anyone who's had any impact on the project"""
    # Set up the screen
    screen = Screen()
    screen.setup(1.0, 1.0)
    screen.bgpic("C:\\Users\\ososb\\Downloads\\pythonProject\\Final Project\\Cash's Project\\Game Menu No Buttons.gif")
    ...
    def scroll_text(text_lines, start_y, step_delay=0.01, font=("Oswald", 60)):
        ...
        for i in range(12.5 * (len(text_lines) + 1)):  # Allow blank frame after last line scrolls off
            pen.clear()  # Clear the screen for smooth animation
            y += 10  # Move all text upwards
            for idx, line in enumerate(text_lines):
                pen.goto(0, y - idx * 75)  # Position each line with a vertical offset
...
def gameHistory():
    os.startfile("Zanzibar's Dice Game.txt")
...
def diceGame():
    ...
    for i in range(numPlayers):
        ...
        playerChips.append(2)  # Initialize 5 chips for each player (2 for presentation)
    ...
    while any(i > 0 for i in playerChips):  # Game ends when no chips are left
        ...
        if any(i == 0 for i in playerChips):  # Check if any player's chips are 0
            ...
            print('Game over')
            winner_index = playerChips.index(min(playerChips))  # Get the index of the winner
            print(f'{playerNames[winner_index]} WINS!')            
